...
export function Gauge({
  ...
  if (greenStart > min) segments.push({ w: pct(greenStart), cls: 'bg-rose-500' })
  segments.push({ w: pct(greenEnd) - pct(greenStart), cls: 'bg-emerald-400' })
  if (greenEnd < max) segments.push({ w: 100 - pct(greenEnd), cls: 'bg-rose-500' })

  const inRange = value !== null && value >= greenStart && value <= greenEnd
  const valueCls = value === null ? 'text-slate-500' : inRange ? 'text-emerald-700' : 'text-rose-600'
