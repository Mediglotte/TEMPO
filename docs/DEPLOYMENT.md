...
d'administration qui nécessitent les droits **admin** du repo (Pierre / compte `Pierre-BALAZ`,
et Félix / `Namsara`, admin depuis juillet 2026).

## Vue d'ensemble

| Brique | Où | Comment |
|---|---|---|
| **Application** (SPA statique) | GitHub Pages — https://pierre-balaz.github.io/TEMPO/ | Auto : chaque push/merge sur `main` déclenche [`deploy.yml`](../.github/workflows/deploy.yml) (build Vite puis publication). Rien à faire. |
...
(`tempo-sync.php`) installé sur un hébergement personnel — hébergement **supprimé en
juillet 2026**, ce chapitre est clos. La synchro passe par un **Cloudflare Worker +
Durable Object** (gratuit, toujours actif, données épinglables en juridiction UE),
**même API** (`GET/POST /room/:code`), code versionné dans [`server/`](../server/).
...
- ~~Désinstaller le plugin WordPress `tempo-sync.php`~~ **Fait** : l'hébergement WordPress de
  Pierre a été supprimé (juillet 2026) — plus aucune dépendance hors GitHub + Cloudflare.

> ⚠️ Si le **compte GitHub qui héberge le repo est renommé**, l'URL Pages change
> (`<compte>.github.io/TEMPO/`) **sans redirection**. Mettre alors à jour : les balises
> `og:url` / `og:image` d'`index.html`, le README, `CLAUDE.md`, ce document, **et la liste
> `ALLOWED_ORIGINS` de `server/src/index.ts`** (puis `npx wrangler deploy`), sinon la synchro
> répond 403 depuis le nouveau domaine.

## Runbook admin (Pierre `Pierre-BALAZ` ou Félix `Namsara`)

État en juillet 2026 : ~~reviews Claude~~ (fait — secret `CLAUDE_CODE_OAUTH_TOKEN` en place,
workflows vérifiés verts), ~~gouvernance à deux~~ (fait — `Namsara` est admin),
~~plugin WordPress~~ (fait — hébergement supprimé). Reste :

1. **Description & sujets du repo** — dans un terminal avec [gh](https://cli.github.com)
   connecté à un compte admin :
   ```bash
   gh repo edit Pierre-BALAZ/TEMPO \
     --description "TEMPO — partition d'urgence : parcours du traumatisé sévère (démo pédagogique, données fictives)" \
     --homepage "https://pierre-balaz.github.io/TEMPO/" \
